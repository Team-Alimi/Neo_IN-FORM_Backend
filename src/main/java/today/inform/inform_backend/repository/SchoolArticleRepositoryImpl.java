...
public class SchoolArticleRepositoryImpl implements SchoolArticleRepositoryCustom {
        ...
        public Page<SchoolArticle> findAllByIdsWithFiltersAndSorting(
                ...
                List<SchoolArticle> content = queryFactory
                                ...
                                .where(
                                                ...
                                                statusFilter(status, today, endingSoonLimit))
                                .orderBy(
        ...
        @Override
        public List<SchoolArticle> findCalendarArticles(List<Integer> categoryIds, Boolean isMyOnly, Integer userId,
        ...
        @Override
        public Page<SchoolArticle> findDailyCalendarArticles(LocalDate selectedDate, List<Integer> categoryIds,
                        Boolean isMyOnly, Integer userId, Pageable pageable) {
                List<SchoolArticle> content = queryFactory
                                .selectFrom(schoolArticle)
                                .leftJoin(schoolArticle.category, category).fetchJoin()
                                .where(
        ...
        private BooleanExpression calendarCategoryFilter(List<Integer> categoryIds, Boolean isMyOnly, Integer userId) {
                if (Boolean.TRUE.equals(isMyOnly)) {
                        if (userId == null)
                                return schoolArticle.articleId.eq(-1);
        ...
        private BooleanExpression dateRangeFilter(LocalDate startDate, LocalDate endDate) {
                ...
                if (endDate != null) {
                        ...
                        BooleanExpression endFilter = schoolArticle.startDate.loe(endDate)
                                        .or(schoolArticle.startDate.isNull());
        ...
        private OrderSpecifier<Integer> createStatusOrder(LocalDate today, LocalDate upcomingLimit,
                        LocalDate endingSoonLimit) {
                NumberExpression<Integer> statusPriority = new CaseBuilder()
                                .when(schoolArticle.startDate.loe(today)
                                                ...
                                                .and(schoolArticle.dueDate.loe(endingSoonLimit)))
                                .then(1)
                                .when(schoolArticle.startDate.loe(today)
                                                .and(schoolArticle.dueDate.gt(endingSoonLimit)
                                                                .or(schoolArticle.dueDate.isNull())))
                                .then(2)
        ...
        private BooleanExpression statusFilter(String status, LocalDate today, LocalDate endingSoonLimit) {
                if (status == null || status.isBlank())
                        return null;

                return switch (status.toUpperCase()) {
                        case "ENDING_SOON" -> schoolArticle.startDate.loe(today)
                                        .and(schoolArticle.dueDate.goe(today))
                                        .and(schoolArticle.dueDate.loe(endingSoonLimit));
                        case "OPEN" -> schoolArticle.startDate.loe(today)
                                        .and(schoolArticle.dueDate.gt(endingSoonLimit)
                                                        .or(schoolArticle.dueDate.isNull()));
                        case "UPCOMING" -> schoolArticle.startDate.gt(today);
                        case "CLOSED" -> schoolArticle.dueDate.lt(today);
                        default -> null;
                };
        }
}
